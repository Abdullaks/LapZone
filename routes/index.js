...
router.get("/deleteAddress/:id", verifyUserLogin, (req, res) => {
  userHelper
    .deleteAddress(req.params.id, req.session.user._id)
    .then((response) => {
      res.redirect("/manageAddress");
    });
});
...
router.post("/removeProductFromCart", (req, res) => {
  userHelper
    .removeProductFromCart(req.body, req.session.user)
    .then((response) => {
...
router.get("/wishList", verifyUserLogin, async (req, res) => {
  ...
  if (wishListCount > 0) {
    const wishListItems = await userHelper.getWishListProducts(
...
router.post("/removeProductFromWishList", (req, res) => {
  userHelper
    .removeProductFromWishList(req.body, req.session.user)
    .then((response) => {
      res.json({ status: true });
    });
});
router.get("/checkOut", verifyUserLogin, async (req, res) => {
  ...
  const [AllAddress, cartItems, netTotal, AllCoupons] = await Promise.all([
...
router.post("/placeOrder", async (req, res) => {
  ...
  userHelper
    .placeOrder(req.body, cartItems, grandTotal, deliveryCharge)
    .then((response) => {
...
router.get("/orderSuccess", (req, res) => {
  const user = req.session.user;
  userHelper
    .getorderProductSuccess(req.session.orderId)
    .then((response) => {
...
router.get("/viewSingleOrderDetails/:id", verifyUserLogin, (req, res) => {
  userHelper
    .getSingleOrderdetails(req.params.id)
    .then((response) => {
      const orderDetails = response;
