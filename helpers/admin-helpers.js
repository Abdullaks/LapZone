...
module.exports = {
  ...
  doAdminLogin: (adminDataa) => {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      if (admin) {
        ...
        console.log('login failed no  such Admin exist');
  ...
  unBlockUser: (userId) => {
    return new Promise(async (resolve, reject) => {
      const user = await userData.findByIdAndUpdate({ _id: userId }, { $set: { block: false } }, { upsert: true })
      resolve(user)
    })
  },
  ...
  addACategory: (Data) => {
    ...
    return new Promise(async (resolve, reject) => {
      const category = await categoryData.findOne({ CategoryName: Data.category })
      console.log(Data.category);
      if (category) {
        reject({ status: false, msg: "this category is already added" })
      } else {
        const addCategory = await new categoryData({ CategoryName: Data.category })
  ...
  deleteCategory: (categoryId) => {
    return new Promise(async (resolve, reject) => {
      const deletecategory = await categoryData.findOneAndDelete({ _id: categoryId })
  ...
  addAProduct: (Data, image_1, image_2, image_3, image_4) => {
    ...
    return new Promise(async (resolve, reject) => {
      const product = await productData.findOne({ ProductName: Data.productname })
      const category = await categoryData.findOne({ CategoryName: Data.category })
  ...
  deleteProduct: (productId) => {
    return new Promise(async (resolve, reject) => {
      const deleteproduct = await productData.findOneAndDelete({ _id: productId })
  ...
  getProductDetails: (productId) => {
    return new Promise(async (resolve, reject) => {
      const productdetails = await productData.findOne({ _id: productId }).lean().then((productdetails) => {
  ...
  updateProduct: (Data, productId, image_1, image_2, image_3, image_4) => {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      const brand = await brandData.findOne({ BrandName: Data.brand })
      const updateproduct = await productData.findOneAndUpdate({ _id: id }, {
        $set: {
          ProductName: Data.productname,
          Brand: brand._id,
          Processor: Data.processorType,
  ...
  getEveryOrders: () => {
    return new Promise(async (resolve, reject) => {
      const EveryOrders = await orderData.find().populate('product.pro_id').lean()
      resolve(EveryOrders)
    })
  },
  getOrderDetails: (orderId) => {
    ...
    return new Promise(async (resolve, reject) => {
      const orderDetails = await orderData.findOne({ _id: id }).populate("product.pro_id").lean()
  ...
  changeOrderStatus: (Data) => {
    return new Promise(async (resolve, reject) => {
      const changeStatus = await orderData.findOneAndUpdate({ _id: Data.orderId, 'product._id': Data.productId },
        {
