...
module.exports = {
      ...
      if (user) {
        ...
        try {
          ...
          const mailDetails = {
            ...
            text: "just random texts ",
            html:
              "<p> Hi " + userDataa.name + " your OTP is" + otpGenerator + "",
          };
          mailTransporter.sendMail(mailDetails, (err, Info) => {
            if (err) {
            } else {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      if (userCart) {
        ...
        if (proExist != -1) {
          cartData
            .updateOne(
              { user: userId, "products.pro_id": productId },
              {
  ...
  getCartProducts: (userId) => {
    return new Promise(async (resolve, reject) => {
      const cartItems = await cartData
        .findOne({ user: userId })
  ...
  getWishListProducts: (userId) => {
    return new Promise(async (resolve, reject) => {
      const wishListItems = await wishListData
  ...
  changePaymentStatus: (orderId) => {
    return new Promise(async (resolve, reject) => {
      const changeStatus = await orderData
        .findOneAndUpdate(
          { _id: orderId },
          {
  ...
  getAllOrders: (user) => {
    return new Promise(async (resolve, reject) => {
      const AllOrders = await orderData
        .find({ user_Id: user })
        .populate("product.pro_id")
        .lean();
      resolve(AllOrders);
  ...
  cancelOrder: (Data) => {
    ...
    return new Promise(async (resolve, reject) => {
      const cancelOrder = await orderData.findOneAndUpdate(
        { _id: orderId, "product.pro_id": Data.productId },
        {
          $set: {
  ...
  deleteAddress: (addressId, userId) => {
    return new Promise(async (resolve, reject) => {
      const removeaddress = await userData.updateOne(
        { _id: userId },
        {
